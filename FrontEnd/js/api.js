...
export function log () {
    ...
    if (loggedIn === "true") {
        ...
        // remplace le tri des travaux par catégories par un bouton pour ouvrir la modale
        setModaleButton()

        // gère la fermeture de modales
        handleModaleClosure()

        // gère la suppression de travaux au clic
        deleteWorks(storedToken)

        // change de modale au clic
        modaleChange()

        // fonction vérifier que le contenu du form est conform
        checkFormContent()

        // fonction pour envoyer le form
        submitForm(storedToken)
...
// gère la suppression de travaux au clic
function deleteWorks(storedToken) {
...
// fonction pour envoyer le form
function submitForm(storedToken) {
